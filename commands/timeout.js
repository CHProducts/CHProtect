...
module.exports = {
    ...
    async execute(interaction) {
        ...
        member.timeout(time * 60 * 1000)
            .then(() => {
                interaction.reply({ embeds: [new EmbedBuilder().addFields({ name: 'Done.', value: '指定したユーザーをタイムアウトしました。' }).setFooter({ text: 'None 0.0.1' }).setColor('#3CB371')], ephemeral: true });
            })
            .catch(() => {
                interaction.reply({ embeds: [new EmbedBuilder().addFields({ name: 'Error', value: '指定したユーザーをタイムアウトできませんでした。' }).setFooter({ text: 'None 0.0.1' }).setColor('ff0000')], ephemeral: true });
            });
    }
};