...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (amount < 1 || amount > 100) {
            return interaction.reply({ embeds: [new EmbedBuilder().addFields({ name: 'Error', value: 'countは1から100の間で指定してください。' }).setFooter({ text: 'None 0.0.1', iconURL: interaction.client.user.displayAvatarURL() }).setColor('#ff0000')], ephemeral: true });
        }
        ...
        interaction.channel.bulkDelete(userMessages)
            .then(() => {
                interaction.reply({ embeds: [new EmbedBuilder().addFields({ name: 'Done.', value: `指定したメンバーの${userMessages.length}件のメッセージを削除しました。` }).setFooter({ text: 'None 0.0.1', iconURL: interaction.client.user.displayAvatarURL() }).setColor('#3CB371')], ephemeral: true });
            })
            .catch(() => {
                interaction.reply({ embeds: [new EmbedBuilder().addFields({ name: 'Error', value: 'メッセージを削除できませんでした。\n時間を空けて再度お試しください。' }).setFooter({ text: 'None 0.0.1', iconURL: interaction.client.user.displayAvatarURL() }).setColor('#ff0000')], ephemeral: true });
            })
    }
};